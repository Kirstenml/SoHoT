...
def get_output_dims(data):
    output_dims = {'sea50': 2, 'sea5E5': 2, 'hyperplane10_0001': 2, 'hyperplane10_001': 2,
                   ...
                   'agrawal_abrupt_drift': 2, 'agrawal_gradual_drift': 2, 'agrawal_real_gradual_drift': 2,
...
def load_data_ht(dataset_name, nrows=None, synthetic_generated=True, oversample_rate=75, distilbert=False, seed=42):
    ...
    if dataset_name.startswith('sea'):
        ...
        if dataset_name[3:].__eq__('50'): noise = 50
        else: noise = 5 * (10 ** 5)
        ...
        for classification_function in range(num_drifts):
            ...
            if classification_function == 0: n_take = nrows // (num_drifts + 1) + width
            else: n_take = nrows // (num_drifts + 1)
    ...
    # ----------------------- Real World Data -------------------------
        ...
        data = pd.read_csv('{}data/realworld/poker/poker-hand-testing.data'.format(DATA_DIR), sep=',', header=None, nrows=nrows)
        ...
        y = [targets[val] for val in y]
        data = zip(river_input_format(x), y)
        # nominal_attributes = [1, 2, 3, 6, 20, 21]
